...
def authenticate_and_fetch_token():
    load_dotenv()
    ...
    if response.status_code == 200:
        ...
        return {'Authorization': 'Bearer ' + token_info['access_token']}
    else:
        st.error("Failed to authenticate.")
        st.error(f"Error Response: {response.text}")
        return None

# Function to generate the report and return it as a DataFrame
def generate_report(auth):
    BASE_URL = os.getenv('BASE_URL')
    report_data = {
        "parameters": {
            "kind": "usage_summary",
    ...
    if response.status_code == 200:
        ...
        while True:
            ...
            if response.status_code == 200:
                stored_reports = response.json().get('items', [])
                if stored_reports:
                    saved_report = next((report for report in stored_reports if report['status'] == 'saved' and report['report_format'] == 'csv_v2_0'), None)
                    if saved_report:
                        stored_report_id = saved_report['id']
                        break
        response = requests.get(f'{BASE_URL}/reports/{report_id}/stored/{stored_report_id}', headers=auth)
        
        if response.status_code == 200:
            csv_content = pd.read_csv(pd.compat.StringIO(response.text))
            return csv_content
        else:
...
auth = authenticate_and_fetch_token()

if auth:
    # Generate the report and get it as a DataFrame
    df_report = generate_report(auth)
